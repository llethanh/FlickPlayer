...
class VideoSource:
    ...
    def _cache_get(self, idx: int) -> np.ndarray | None:
        ...
        No access-time tracking — eviction is **strict insertion
        order FIFO** (see ``_cache_put``). For forward playback the
        oldest-inserted frame is always the furthest behind the
        playhead, which is what the user expects to drop first.
        """
    ...
    def _cache_put(self, idx: int, arr: np.ndarray) -> None:
        """Store ``arr`` for ``idx``. Idempotent. Evicts the
        **oldest-inserted** frame until back under budget (strict
        FIFO).

        For forward playback the prefetch worker writes ahead of the
        playhead, so the oldest insertion is always the frame
        furthest behind the playhead — matching the user's mental
        model of "the oldest cached frame goes first" and producing
        a contiguous cache window that slides forward with the
        playhead.

        For scrub-back, freshly decoded frames behind the playhead
        also enter at the tail of the FIFO, so the user's
        most-recent-interest is preserved naturally — what gets
        evicted is whatever has been sitting in cache the longest.

        Distance-based eviction (the v1.8.3 → v1.8.4 policy) was
        replaced after a 2026-06-18 user report on a 2252-frame .mov:
        the linear-cost function with a 60-frame scrub-back window
        was correct on paper but the actual eviction trace showed
        non-contiguous holes forming behind the playhead. FIFO is
        simpler, has no tuning knobs, and matches the documented
        expectation.
        """
        ...
        with self._cache_lock:
            if idx in self._frame_cache:
                # Re-put with same data is a no-op — would otherwise
                # move the key to the tail and break FIFO semantics.
                return
            ...
            while (
                ...
                # Python 3.7+ dict preserves insertion order, so the
                # first key in the iter is always the oldest. O(1).
                oldest = next(iter(self._frame_cache))
                dropped = self._frame_cache.pop(oldest)
